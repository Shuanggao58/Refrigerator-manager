...
def inference(input_tensor, train, regularizer):
    ...
    # second convolutional neural network layer (third layer)
    ...
    # third convolutional neural network layer (fourth layer)
    ...
    with tf.variable_scope('layer5-fc1'):
        ...
        # dropout, we can reduce overfit here
        if train:
...
for i in range(epoch):
    ...
    for x_train_a, y_train_a in get_feed_data(x_train, y_train, batch_size, shuffle=True):
        ...
        train_acc += ac
        n_batch += 1
    print("Ep: %02d" % (i + 1) + " train loss: %f, train accuracy: %f" % (np.sum(train_loss) / n_batch, np.sum(train_acc) / n_batch))
    ...
    for x_val_a, y_val_a in get_feed_data(x_val, y_val, batch_size, shuffle=False):
        ...
        n_batch += 1
    print("Ep: %02d" % (i + 1) + " validation loss: %f, validation accuracy: %f" % (np.sum(val_loss) / n_batch, np.sum(val_acc) / n_batch))
